...
fn compile(
    ...
    config
        ...
        .protoc_arg("grpc_api_configuration=proto/v1/api_config_http.yaml,output_format=yaml,allow_merge=true,merge_file_name=brindexer,json_names_for_fields=false")
        .bytes(["."])
        .btree_map(["."])
        .type_attribute(".", "#[actix_prost_macros::serde(rename_all=\"snake_case\")]")
        // .field_attribute(
        //     ".blockscout.brindexer.v1.<MessageName>.<DefaultFieldName>",
        //     "#[serde(default)]"
        // )
        ;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile(
        &["proto/v1/brindexer.proto", "proto/v1/health.proto"],
        &["proto"],
        gens,
    )?;
    Ok(())
